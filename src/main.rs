...
use render_loop::{FPSLimiter, RenderLoop};
...
lazy_static::lazy_static! {
    static ref SHADERS: rendy::shader::ShaderSetBuilder = shaders::load_shaders(concat!(env!("CARGO_MANIFEST_DIR"), "/src/shader"));
    ...
    B: hal::Backend,
{
    ...
    fn load_shader_set(&self, factory: &mut Factory<B>, _scene: &Scene<B>) -> rendy_shader::ShaderSet<B> {
    ...
    fn vertices(
        ...
        //log::info!("{:#?}", SHADER_REFLECTION.cache.vertices);
        log::info!("{:#?}", SHADER_REFLECTION.attributes(&["model"]).unwrap());
        log::info!("{:#?}", res);
        return res;
    }
    ...
    fn build<'a>(
        ...
        let align = factory.physical().limits().min_uniform_buffer_offset_alignment;

        let buffer = factory
            .create_buffer(
                BufferInfo {
                    size: buffer_frame_size(align) * frames as u64,
                    usage: hal::buffer::Usage::UNIFORM | hal::buffer::Usage::INDIRECT | hal::buffer::Usage::VERTEX,
                },
                Dynamic,
            )
            .unwrap();
    ...
    B: hal::Backend,
{
    ...
    fn prepare(
        ...
        if !scene.objects.is_empty() {
            unsafe {
                factory
                    .upload_visible_buffer(&mut self.buffer, models_offset(index, self.align), &scene.objects[..])
                    .unwrap()
            };
        }
    ...
    fn draw(
        ...
        log::info!("Drawing");
        unsafe {
            encoder.bind_graphics_descriptor_sets(layout, 0, Some(self.sets[index].raw()), std::iter::empty());

            let vertex = [SHADER_REFLECTION.attributes(&["position", "color", "normal"]).unwrap()];

            // Bind the position/color/normal data to vertex buffer 0
            scene
                .object_mesh
                .as_ref()
                .unwrap()
            ...
            // Bind the per model matrix data to vertex buffer 1
...
fn main() {
    ...
    let mut render_loop = render_loop::RenderLoop::<Backend>::new(config, event_loop);

    let (mut factory, mut families) = (&mut render_loop.factory, &mut render_loop.families);
    ...
    let depth = graph_builder.create_image(
        ...
        Some(hal::command::ClearValue::DepthStencil(hal::command::ClearDepthStencil(
            1.0, 0,
        ))),
    );
    ...
    let vertices: Vec<_> = icosphere
        ...
        .map(|v| PosColorNorm {
            position: v.pos.into(),
            color: [(v.pos.x + 1.0) / 2.0, (v.pos.y + 1.0) / 2.0, (v.pos.z + 1.0) / 2.0, 1.0].into(),
    ...
    loop {
        ...
        if elapsed > std::time::Duration::new(20, 0) || scene.objects.len() == MAX_OBJECTS {
            break;
        }
        if !render_loop.run() {
            let frames = render_loop.frame - start;
        ...
        graph.run(&mut render_loop.factory, &mut render_loop.families, &scene);
...

        {
            let frames = render_loop.frame - start;
            let nanos = elapsed.as_nanos() as u64;
            let fps = frames * 1_000_000_000 / nanos;
            log::info!("FPS: {:#?} {:#?}", render_loop.frame, fps);
        }
    ...
    graph.dispose(&mut render_loop.factory, &scene);
}
